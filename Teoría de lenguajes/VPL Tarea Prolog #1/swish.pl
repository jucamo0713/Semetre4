...
tio(X, Y) :-  hijo(X,AB), abuelo(AB,Y), hombre(X), padre(P, Y), X\=P.
tia(X, Y) :- hijo(X,AB), abuelo(AB,Y), mujer(X), madre(M, Y), X\=M.

sobrino(X, Y) :- hijo(X,P), hermanos(PD,Y).
primos(X,Y) :- abuelo(X,AB), hijo(PD,AB), padre(P, Y), PD\=P, madre(M, Y), PD\=M, hermanos(H,PD),hijo(Y,H).

bisabuelo(BAP, X) :- padre(BAP,AB),abuelo(AB,PE).
bisabuela(BAP, X) :- .
